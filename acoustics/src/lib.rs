...
// HORIZON[P3]: Compute a temperature-based pitch factor and feed it through an Exponential Moving Average (EMA) to avoid abrupt, "robotic" pitch changes.
// HORIZON[P3]: Use a band-limited oscillator (e.g., wavetable) for the Chebyshev exciter to prevent aliasing at high pitches.
...
fn try_open_cpal_stream(
    ...
    let out_stream = match out_config.sample_format() {
        cpal::SampleFormat::F32 => speaker.build_output_stream(
            ...
            move |data: &mut [f32], _: &_| {
                ...
                let gpu_thermal_celsius =
                    f32::from_bits(state.gpu_thermal_celsius.load(Ordering::Relaxed));
                ...
                data.iter_mut()
                    ...
                    .for_each(|(_sample_idx, sample)| {
                        ...
                        let out =
                            b0 * drone + b1 * iir_z1 + b2 * iir_z2 - a1 * iir_z1 - a2 * iir_z2;
                        iir_z2 = iir_z1;
                        iir_z1 = out;
    ...
    loop {
        ...
        if let Ok(chunk) = consumer.read_chunk(1024) {
            ...
            // HORIZON[P2]: Implement real-time multi-speaker diarization using SIMD-accelerated clustering for deterministic sub-10ms latency.
            ...
            let rms = if mean_sq > 0.0 {
                mean_sq * q_rsqrt(mean_sq)
            } else {
                0.0
            };
