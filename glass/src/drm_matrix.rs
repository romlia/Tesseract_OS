...
use rust_drm::control::{Device as ControlDevice, dumbbuffer::DumbBuffer, framebuffer, PageFlipFlags};
...
impl HolographicManifold {
    pub async fn ignite_bare_metal() -> anyhow::Result<Self> {
        ...
        let targets = ["/dev/dri/card1", "/dev/dri/card0", "/dev/dri/card2", "/dev/dri/card3"];
        for path in targets {
            if let Ok(device) = seat.open_device(&path) {
                ...
                if let Ok(_) = GbmDevice::new(file.try_clone().unwrap()) {
                    drm_node = Some(file);
                    tracing::info!("Hardware Seizure Successful (via logind/seatd) on: {}", path);
                    break;
                } else {
        ...
        let drm_node = drm_node.ok_or_else(|| anyhow::anyhow!("Hardware Seizure Failed: No suitable DRM nodes found via libseat"))?;
        
        let card = DrmCard(drm_node.try_clone().unwrap());
        use rust_drm::control::Device;
        let res = card.resource_handles().map_err(|e| anyhow::anyhow!("Failed to get DRM resources: {:?}", e))?;
        ...
        let display_mode = active_mode.ok_or_else(|| anyhow::anyhow!("No active display mode found"))?;
        let crtc = active_crtc.ok_or_else(|| anyhow::anyhow!("No active CRTC found"))?;
        let connector = active_connector.ok_or_else(|| anyhow::anyhow!("No active connector found"))?;
        ...
        let adapter = adapter_future.await.map_err(|_| anyhow::anyhow!("Failed to acquire compatible WGPU adapter"))?;

        let device_req: wgpu::DeviceDescriptor = wgpu::DeviceDescriptor::default();
        let (device, queue): (wgpu::Device, wgpu::Queue) = adapter.request_device(&device_req).await.map_err(|e| anyhow::anyhow!("Failed to acquire WGPU device queue: {:?}", e))?;
        ...
        Ok(Self { device, queue, adapter, drm_node, display_mode, crtc, connector, card, double_buffer, _seat: seat })
    }
    
    // Phase 10: Bare-Metal Page Flipping & V-Sync
    // TODO: DRM/KMS Mode-Setting Integration (Use kmscon or DRM library for smooth hand-off to avoid flicker)
    pub fn present_frame(&mut self, wgpu_pixels: &[u8]) -> anyhow::Result<()> {
        ...
        let mut mapping = self.card.map_dumb_buffer(&mut next_buffer)
